...
Such operations can be achieved by calling a single endpoint: [POST /sales/invoice])(/openapi/sales/invoice) with `basicInformation > statusId` set to `ISSUED`
...
Using [POST /sales/pdfUrl](/openapi/sales/pdfUrl) endpoint you can retrieve a pre-signed url that gives you access to the file for 10 minutes

### Sending my invoice to recipient
Using [POST /sales/invoice/send](/openapi/sales/send) endpoint you can order an e-mail to be sent to your contractor using either the data saved in the system (Contractor info saved along with the invoice) or using an e-mail address of your choice directly.
